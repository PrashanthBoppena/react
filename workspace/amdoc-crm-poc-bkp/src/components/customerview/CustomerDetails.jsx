...
//import { getCustomerRequest, getCustomerAccountsRequest } from '../../redux/actions';
  ...
  return (
    <Container>
                ...
                <Box p={2}>
                  ...
                  <Box mt={2}>
                    <Box display="flex" alignItems="center" margin="8px 0">
                      ...
                      <Typography variant="body1"><strong>Email:</strong> {customer.result[0].contactList && customer.result[0].contactList[0]?.primaryEmail}</Typography>
                    </Box>
                    <Box display="flex" alignItems="center" margin="8px 0">
                      <PhoneIcon style={{ marginRight: '16px' }} />
                      <Typography variant="body1"><strong>Mobile:</strong> {customer.result[0].contactList && customer.result[0].contactList[0]?.primaryMobile}</Typography>
                    </Box>
                    <Box display="flex" alignItems="center" margin="8px 0">
                      <LocationOnIcon style={{ marginRight: '16px' }} />
                      <Typography variant="body1">
                        <strong>Address:</strong> {customer.result[0].addressList && customer.result[0].addressList[0]?.addressLine1}, {customer.result[0].addressList && customer.result[0].addressList[0]?.addressCity}, {customer.result[0].addressList && customer.result[0].addressList[0]?.addressState}
                      </Typography>
                    </Box>
                    <Box display="flex" alignItems="center" margin="8px 0">
                      <AccountBalanceWalletIcon style={{ marginRight: '16px' }} />
                      <Typography variant="body1"><strong>Billing Cycle:</strong> {customer.result[0].billCycle}</Typography>
                    </Box>
                    <hr style={{ borderTop: '1px dashed #ccc', margin: '10px 0' }} /> {/* Divider line */}
                    <Box mt={2}>
                      <Grid container spacing={1}>
                        ...
                        <Grid item xs={6}>
                          <Button variant="contained" fullWidth style={{ marginBottom: '10px', backgroundColor: '#64b5f6', color: 'white' }}>
                            <RecentActorsSharp /> Re-Charge
                          </Button>
                        </Grid>
                        <Grid item xs={6}>
                          <Button variant="contained" fullWidth style={{ marginBottom: '10px', backgroundColor: '#64b5f6', color: 'white' }}>
                            <AbcOutlined /> Pay Bills
                          </Button>
                        </Grid>
                        <Grid item xs={6}>
      ...
      <Paper style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%', background: '#ffefba', background: '-webkit-linear-gradient(to right, #ffefba, #ffff)', background: 'linear-gradient(to right, #ffff, #ffefba)' }}>
        <Box p={2}>
          <Typography variant="h6" style={{ color: '#ff0000' }}>My Accounts</Typography>
          <Tabs value={tabValue} onChange={handleTabChange} indicatorColor="primary">
            <Tab label={<span style={{ fontWeight: 'bold' }}>All</span>} />
            <Tab label={<span style={{ fontWeight: 'bold' }}>By Status</span>} />
            <Tab label={<span style={{ fontWeight: 'bold' }}>By Type</span>} />
          </Tabs>
          {loadingAccounts ? (
            <CircularProgress />
          ) : errorAccounts ? (
            <Typography color="error">Error loading accounts</Typography>
          ) : (
            <List>
        ...
        <Grid item xs={12} md={4}>
          <Paper style={{ 
            ...
            <WalletSection wallet={wallet} profile={profile} />
          </Paper>
        </Grid>
      </Grid>
    </Container>
